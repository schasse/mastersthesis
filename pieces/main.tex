...
% (maintainance is a lot of work and) is easily disregarded in favor of velocity
...
software product. Agile and continuous practices focus primarily software
development. Those practices altered software development to deliver features faster into
production. Fitzgerald asses that the focus of current research is primarily on the
delivery process. And Fowler argues velocity kills agility TODO. Focusing on development
velocity only has a major drawback. Companies easily disregard the operation of the
software product in production.

% devops/microservices aproach successful development and operation
The DevOps TODO approach and the microservice culture TODO create practices to solve this
...
it, you run it` TODO, which creates the awareness of the teams to not only be responsible
for development and deliver features whereas they are also responsible to operate their
product in production. This is because operation is an essential part of a successful
software product.
...
work related to running the software product in production. It includes the configuration
of the servers. It includes monitoring of performance, errors and security issues. And it
...
% ops work is important becuase it involves risk.
...
Companies like RedHat or Ubuntu provide enterprise support to open source projects. Those
companies bundle those open source projects, care about the bugs and security issues and
provide tools to upgrade from one version to another easily. As a result companies with a
software product can outsource this work. Those company saves operations and maintenance
work and is able provide a better quality of their software product.

% is risky (see above?)
If we do not do maintenance properly, it results in a risk of security
...
at once. Consequently this means we need to update more often and the work of updates
itself is more.
...
reusable software modules make it easier and faster for developers to reach their
goals. We conclude due to those facts that dependencies are becoming more.

% containerization supports microservice approach
With containerization comes a big change for dependencies. Containerization is a further
step of isolation. The operating system is able to isolate cpu and memory for every
process. Yet with containerization the operating system can also isolate the disk between
processes, which means it can isolate the installed dependencies for every process. This
is the supporting technology for microservices, which as we previously described have the
advantage to be flexible when it comes to updating dependencies.

% containerization takes out risk for updates
Containerization takes out the risk of updating the isolated dependencies of a
process. Morris describes phoenix replacement in immutable infrastructure TODO. Kubernetes
has the approach of rolling updates, which implement the same practices. A containerized
ecosystem is typically immutable. Rather than changing a running container, we would
create a new container based on an updated image including the changes to deploy. After
the new container was started, we can kill the old container. Usually an image repository
holds every built container image version. Those saved images allow us to easily rollback
to a previous version. Hence updates are less risky in a containerized immutable
infrastructure.

% more updates less risk -> automation
...
In acount of the dependencies there is a responsibility shift towards developers. In the
past, development teams delivered code and depended on the installed dependencies.
Operation teams have been responsible of maintaining those dependencies. Virtualization
made a change. Operation teams operating a virtual infrastructure operate and maintain the
hardware and software of this infrastructure. They do not maintain the specific
dependencies code developed by the development team. The development team can choose which
dependencies they want to have. Accordingly there is a clear isolation between the
dependencies of the virtual machine infrastructure and the development
team.
...
teams. Nevertheless it becomes easier to maintain a dynamic virtual infrastructure
compared to a bare metal infrastructure. And the easier it becomes, the more it enables
developers to operate their software product themselves. Containerization techniques even
potentiates the ease of managing dependencies. So the responsibility of the dependencies
of a software shifts resultantly from the operation team to the development team.
...
you change only a few replicated parts of the production environment to reduce risk. There
is also shadow releasing, where users are not affected by the change, which goes into
...
You could argue that those DevOps practices already exist and connect development and
operations of a software product. However the focus lies mostly on testing the quality of
a change in a lean fashion and to reduce risk of a change in production. It is true that
those practices help to deliver changes faster to production and help to detect issues at
low risk. Nevertheless monitoring and reacting to errors requires manual work.
...
% another time practices exist, lack of (need to rewrite)
...
that DevOps focuses on operating a software product in production. This is obvious as it
includes monitoring practices. But as I mentioned, there is lack though of
\emph{automatable} practices for operating and maintaining the software product in
production.

% contribution of the masterthesis
...
\section{Post Release Testing extends the Continuous Delivery Pipeline to support
