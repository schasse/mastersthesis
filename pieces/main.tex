...
Chapter~\ref{chap:problem} elaborates on the problem of the amount maintenance work and
...
shortly explains the major kubernetes resources on the example of 3 tier webapp.

\paragraph{\ref{chap:macro} \nameref{chap:macro}}
Chapter~\ref{chap:macro} describes conceptual macro view of nonfunctional production
...
Deployer is implemented in ruby~\cite{ruby} and since it is a webserver,
sinatra~\cite{sinatra} is its framework. Deployer uses git~\cite{git} and
subversion~\cite{svn} to communicate with either git or subversion as the control version
system. Moreover deployer has a plugin which sends messages to bugsnag~\cite{bugsnag} and
slack~\cite{slack}, to inform the developer in case it identifies a problem. Deployer uses
kubectl to interact with kubernetes~\cite{kubernetes} master. Deployer itself is
containerized with docker~\cite{docker} and its natural hosting solution would be
kubernetes. We published deployer on github~\cite{deployer} under GPL-3.0, a free software
license.

The monitoring system, which I used in the evaluation, is datadog~\cite{datadog}, a
commercial service. Nevertheless prometheus~\cite{prometheus} is also a suitable open
source solution as monitoring system. The continuous integration systems we use for the
evaluation are codeship~\cite{codeship} and jenkins~\cite{jenkins}. As an image repo, we
use docker hub~\cite{dockerhub}.
