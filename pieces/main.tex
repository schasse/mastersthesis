...
At first common practices from software engineering. Initially version control with a
\emph{version control system}. Humble explains what is important in version
control~\cite{cd_humble_config}.

Next the reader should know about continuous integration as Fowler
propagates~\cite{ci_fowler} and what a \emph{continuous
  integration system} is.

I refer \emph{DevOps}, which Bass et.~al defines~\cite{devops_definition}.

The reader should know what \emph{continuous delivery} is and how continuous delivery
relates to DevOps~\cite{cd_wolff_devops}.

Important practices from continuous delivery are prerequisite: \emph{canary releasing},
\emph{zero downtime deploys} and \emph{continuous deployments}, which Humble
illustrates~\cite{cd_humble_deploy}.

I require the reader to have an idea of monitoring in a distributed system as Ewaschuk
describes in ``Site Reliability Engineering''~\cite{sre_monitoring} and how Bass et.~al
illustrates the design of a \emph{monitoring system}~\cite[127-154]{devops_monitoring}.

I presume the reader to know the concepts of dynamic infrastructures and infrastructures
defined as code\cite{infra_as_code_platforms}. Morris explains how to common software
engineering practices for infrastructure~\cite{infra_as_code_se_practices}.

The concepts of cluster management and how Google manages its clusters are
important~\cite{borg}. Hence those concepts heavily influenced the design of
\emph{kubernetes}~\cite{borg_kubernetes}.

\subsection{Technologies}

The next references are practical introductions to the concepts of the technologies, which
I extensively use in this masterthesis. It is necessary to know what docker \emph{image}
and \emph{containers} are~\cite{docker_orientation}\cite{docker_orientation2}.

It helps to have an idea of the architecture of kubernetes. Beda gives a good
overview~\cite{kubernetes_architecture2}.

Absolutely relevant for the next chapters are the kubernetes resources. Before reading
chapter 4 and 5 the reader should know what \emph{pods}~\cite{pod}, \emph{deployments}
(the new replicationcontroller)~\cite{replicationcontroller} and
\emph{services}~\cite{service}.
